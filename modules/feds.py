...
import Gabby.modules.sql.feds_sql as sql
from Gabby import (
    DRAGONS,
    EVENT_LOGS,
    LOGGER,
...
from Gabby.modules.disable import DisableAbleCommandHandler
from Gabby.modules.helper_funcs.alternate import send_message
from Gabby.modules.helper_funcs.chat_status import is_user_admin
from Gabby.modules.helper_funcs.decorators import Gabbycmd
from Gabby.modules.helper_funcs.extraction import (
    extract_unt_fedban,
    extract_user,
    extract_user_fban,
)
from Gabby.modules.helper_funcs.string_handling import markdown_parser
    ...
    if args:
        if args[0] == "json":
            ...
            with BytesIO(str.encode(backups)) as output:
                output.name = "Gabby_fbanned_users.json"
                update.effective_message.reply_document(
                    document=output,
                    filename="Gabby_fbanned_users.json",
        ...
        if args[0] == "csv":
            ...
            for users in getfban:
                ...
                backups += "\n"
            with BytesIO(str.encode(backups)) as output:
                output.name = "Gabby_fbanned_users.csv"
                update.effective_message.reply_document(
                    document=output,
                    filename="Gabby_fbanned_users.csv",
...
@gabbycallback(pattern=r"fed_help_")
