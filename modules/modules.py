...
from gabby_main import dispatcher, telethon
from gabby_main.__main__ import (
    CHAT_SETTINGS,
    DATA_EXPORT,
    DATA_IMPORT,
    HELPABLE,
...
from .modules.helper_funcs.decorators import gabby_maincmd


@gabbycmd(command="load")
@dev_plus
def load(update: Update, context: CallbackContext):
    ...
    try:
        imported_module = importlib.import_module(f"gabby_main.gabby_modules.{text}")
    except Exception:
    ...
    if "__handlers__" in dir(imported_module):
        ...
        for handler in handlers:
            if not isinstance(handler, tuple):
                ...
                telethon.add_event_handler(callback, telethon_event)
            else:
...
def unload(update: Update, context: CallbackContext):
    ...
    try:
        imported_module = importlib.import_module(f"gabby_main.modules11.{text}")
    except Exception:
    ...
    if "__handlers__" in dir(imported_module):
        ...
        for handler in handlers:
            ...
            if not isinstance(handler, tuple):
                ...
                callback, telethon_event = handler
                telethon.remove_event_handler(callback, telethon_event)
            else:
                handler_name, priority = handler
...
@gabby_maincmd(command="listmodules")
@sudo_plus
def listmodules(update: Update, context: CallbackContext):
    ...
    for helpable_module in HELPABLE:
        ...
        file_name = file_info.__name__.rsplit("gabby_main.modules11.", 1)[1]
